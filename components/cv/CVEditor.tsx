...
export const CVEditor: React.FC<CVEditorProps> = ({ data, onChange }) => {
  ...
  // États d'accordéons pour rendre l'édition modulaire et réduire le défilement
  const [personalSection, setPersonalSection] = useState<'identity' | 'contact' | 'summary' | null>('identity');
  const [expandedExpIndex, setExpandedExpIndex] = useState<number | null>(0);
  const [expandedEduIndex, setExpandedEduIndex] = useState<number | null>(0);
  const [expandedSkillSection, setExpandedSkillSection] = useState<'skills' | 'languages' | null>('skills');
  const [expandedProjIndex, setExpandedProjIndex] = useState<number | null>(0);
  const [expandedProjSection, setExpandedProjSection] = useState<'projects' | 'certifications' | null>('projects');
  const [expandedCustSection, setExpandedCustSection] = useState<'presets' | 'layouts' | 'colors' | 'photo' | 'typography' | null>('presets');
  ...
  const addWorkExperience = () => {
    ...
    setExpandedExpIndex(data.workExperiences.length);
  };
  ...
  const addEducation = () => {
    ...
    setExpandedEduIndex(data.educations.length);
  };
  ...
  const addProject = () => {
    ...
    setExpandedProjIndex(data.projects.length);
  };
  ...
  return (
    <div className="border overflow-hidden flex flex-col h-full rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-200 bg-white border-slate-200/90 text-slate-900">
      ...
      <div className="p-5 overflow-y-auto max-h-[calc(100vh-140px)] space-y-6">
        ...
        {activeTab === 'personal' && (
          <div className="space-y-4">
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setPersonalSection(personalSection === 'identity' ? null : 'identity')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                    <User className="w-4 h-4" />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">📸 Photo & Identité</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium truncate max-w-[250px] sm:max-w-[320px]">
                      {data.personalInfo.fullName || 'Renseignez votre nom'} {data.personalInfo.title ? `• ${data.personalInfo.title}` : ''}
                    </p>
                  </div>
                </div>
                {personalSection === 'identity' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: personalSection === 'identity' ? 'auto' : 0, opacity: personalSection === 'identity' ? 1 : 0 }}
            ...
            {/* Section 2: Coordonnées & Réseaux */}
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setPersonalSection(personalSection === 'contact' ? null : 'contact')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">📞 Coordonnées & Réseaux</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium truncate max-w-[250px] sm:max-w-[320px]">
                      {data.personalInfo.email || data.personalInfo.phone || 'Email, téléphone, localisation, réseaux'}
                    </p>
                  </div>
                </div>
                {personalSection === 'contact' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: personalSection === 'contact' ? 'auto' : 0, opacity: personalSection === 'contact' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">Date de naissance <span className="text-[10px] text-slate-400 font-normal">(Facultatif)</span></label>
                    <input
                      type="text"
                      value={data.personalInfo.birthDate || ''}
                      onChange={(e) => updatePersonalInfo('birthDate', e.target.value)}
                      placeholder="ex: 12 Janvier 1995"
                      className="w-full text-xs p-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none transition-all bg-white"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">Lieu de naissance <span className="text-[10px] text-slate-400 font-normal">(Facultatif)</span></label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">Nationalité <span className="text-[10px] text-slate-400 font-normal">(Facultatif)</span></label>
                    <input
                      type="text"
                      value={data.personalInfo.nationality || ''}
                      onChange={(e) => updatePersonalInfo('nationality', e.target.value)}
                      placeholder="ex: Française, Congolaise, etc."
                      className="w-full text-xs p-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none transition-all bg-white"
                    />
                  </div>
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setPersonalSection(personalSection === 'summary' ? null : 'summary')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">📝 Résumé Professionnel</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium truncate max-w-[250px] sm:max-w-[320px]">
                      {data.personalInfo.summary || 'Courte présentation de votre parcours'}
                    </p>
                  </div>
                </div>
                {personalSection === 'summary' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: personalSection === 'summary' ? 'auto' : 0, opacity: personalSection === 'summary' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white space-y-4">
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-between p-4 text-left transition-colors bg-slate-50/50"
                      >
                        <div className="flex items-center gap-3 pr-4 truncate min-w-0">
                          <div className={`p-2 rounded-xl shrink-0 ${isExpanded ? 'bg-slate-900 text-white' : 'bg-slate-200/60 text-slate-700'}`}>
                          ...
                          <div className="truncate">
                            ...
                            <p className="text-[10px] text-slate-500 mt-0.5 font-semibold truncate">
                              {exp.company || 'Entreprise non renseignée'}
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              if (isExpanded) setExpandedExpIndex(null);
                            }}
                        ...
                        <div className="p-4 bg-white border-t border-slate-100 space-y-4">
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              rows={4}
                              className="w-full text-xs p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 bg-white leading-relaxed transition-all font-sans"
                            />
                          </div>
                        </div>
                      </motion.div>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-3 pr-4 truncate min-w-0">
                          ...
                          <div className="truncate">
                            ...
                            <p className="text-[10px] text-slate-500 mt-0.5 font-semibold truncate">
                              {edu.institution || 'Établissement non renseigné'}
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              if (isExpanded) setExpandedEduIndex(null);
                            }}
                            className="text-slate-400 hover:text-red-600 p-1.5 rounded-xl hover:bg-red-50 transition-colors"
                      ...
                      <motion.div
                        ...
                        className="overflow-hidden"
                      >
                        <div className="p-4 bg-white border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
        ...
        {activeTab === 'skills' && (
          <div className="space-y-4">
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedSkillSection(expandedSkillSection === 'skills' ? null : 'skills')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">🛠 Compétences Clés</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">
                      {data.skills.length} compétence{data.skills.length > 1 ? 's' : ''} répertoriée{data.skills.length > 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
                {expandedSkillSection === 'skills' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedSkillSection === 'skills' ? 'auto' : 0, opacity: expandedSkillSection === 'skills' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Liste des compétences</span>
                  ...
                  {data.skills.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-4 italic">Aucune compétence ajoutée.</p>
                  ) : (
                    <div className="space-y-2">
                      {data.skills.map((skill, index) => (
                        <div key={skill.id} className="flex items-center gap-2 bg-slate-50/50 p-1.5 rounded-xl border border-slate-100">
                          <input
                            type="text"
                            value={skill.name}
                            onChange={(e) => updateSkill(index, 'name', e.target.value)}
                            placeholder="Nom de la compétence (ex: React.js)"
                            className="flex-1 text-xs p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none bg-white transition-all font-medium"
                          />
                          <select
                            value={skill.level}
                            onChange={(e) => updateSkill(index, 'level', e.target.value)}
                            className="text-xs p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none bg-white font-semibold text-slate-700"
                          >
            ...
            {/* Section 2: Langues Parlées */}
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedSkillSection(expandedSkillSection === 'languages' ? null : 'languages')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">🗣 Langues Parlées</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">
                      {data.languages.length} langue{data.languages.length > 1 ? 's' : ''} répertoriée{data.languages.length > 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
                {expandedSkillSection === 'languages' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedSkillSection === 'languages' ? 'auto' : 0, opacity: expandedSkillSection === 'languages' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Liste des langues</span>
                    <button
                      type="button"
                      onClick={addLanguage}
                      className="px-3.5 py-1.5 text-xs font-bold bg-slate-900 hover:bg-black text-white rounded-full flex items-center gap-1.5 shadow-2xs hover:scale-102 transition-all"
                  ...
                  {data.languages.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-4 italic">Aucune langue ajoutée.</p>
                  ) : (
                    <div className="space-y-2">
                      {data.languages.map((lang, index) => (
                        <div key={lang.id} className="flex items-center gap-2 bg-slate-50/50 p-1.5 rounded-xl border border-slate-100">
                          <input
                            type="text"
                            value={lang.name}
                          ...
                          <select
                            ...
                            className="w-40 text-xs p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none bg-white font-semibold text-slate-700"
                          >
                            <option value="Débutant">Débutant</option>
                            <option value="Intermédiaire">Intermédiaire</option>
        ...
        {activeTab === 'projects' && (
          <div className="space-y-4">
            {/* Section 1: Projets & Réalisations */}
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedProjSection(expandedProjSection === 'projects' ? null : 'projects')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">🚀 Projets & Réalisations</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">
                      {data.projects.length} projet{data.projects.length > 1 ? 's' : ''} répertorié{data.projects.length > 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
                {expandedProjSection === 'projects' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedProjSection === 'projects' ? 'auto' : 0, opacity: expandedProjSection === 'projects' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Liste des projets</span>
                    <button
                      type="button"
                      onClick={addProject}
                      className="px-3.5 py-1.5 text-xs font-bold bg-slate-900 hover:bg-black text-white rounded-full flex items-center gap-1.5 shadow-2xs hover:scale-102 transition-all"
                  ...
                  {data.projects.length === 0 ? (
                    <div className="text-center py-6 bg-slate-50 border border-dashed border-slate-200 rounded-xl">
                      <p className="text-xs text-slate-400 italic">Aucun projet ajouté pour le moment.</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {data.projects.map((proj, index) => {
                        const isProjExpanded = expandedProjIndex === index;
                        return (
                          <div
                            key={proj.id}
                          ...
                          >
                            <button
                              type="button"
                              onClick={() => setExpandedProjIndex(isProjExpanded ? null : index)}
                              className="w-full flex items-center justify-between p-3 bg-slate-50/40 text-left transition-colors"
                            >
                              <div className="truncate min-w-0 pr-4">
                                <h5 className="text-xs font-bold text-slate-700 truncate">
                                  {proj.title || 'Nouveau projet'}
                                </h5>
                                <p className="text-[10px] text-slate-400 mt-0.5 truncate">
                                  {proj.link || 'Sans lien renseigné'}
                                </p>
                              </div>
                              <div className="flex items-center gap-1.5 shrink-0">
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    deleteProject(index);
                                    if (isProjExpanded) setExpandedProjIndex(null);
                                  }}
                            ...
                            <motion.div
                              initial={false}
                              animate={{ height: isProjExpanded ? 'auto' : 0, opacity: isProjExpanded ? 1 : 0 }}
                            ...
                            >
                              <div className="p-3 bg-white border-t border-slate-100 space-y-3">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                  <div>
                                    <label className="block text-[10px] font-semibold text-slate-500 mb-1">Titre du projet</label>
                                    <input
                                      type="text"
                                      value={proj.title}
                                      onChange={(e) => updateProject(index, 'title', e.target.value)}
                                      placeholder="Titre du projet"
                                      className="w-full text-xs p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none"
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-[10px] font-semibold text-slate-500 mb-1">Lien du projet</label>
                                    <input
                                      type="text"
                                      value={proj.link}
                                ...
                                <div>
                                  <label className="block text-[10px] font-semibold text-slate-500 mb-1">Description des travaux</label>
                                  <textarea
                                    value={proj.description}
                                    onChange={(e) => updateProject(index, 'description', e.target.value)}
                                    placeholder="Description du projet et réalisations clés..."
                                    rows={3}
                                    className="w-full text-xs p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none leading-relaxed"
                                  />
                                </div>
                                <div>
                                  <label className="block text-[10px] font-semibold text-slate-500 mb-1">Technologies utilisées</label>
                                  <input
                                    type="text"
                                    value={proj.technologies}
                                    onChange={(e) => updateProject(index, 'technologies', e.target.value)}
                                    placeholder="React, Tailwind, TypeScript, etc."
                                    className="w-full text-xs p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none"
                                  />
                                </div>
                              </div>
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedProjSection(expandedProjSection === 'certifications' ? null : 'certifications')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">🏆 Certifications</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">
                      {data.certifications.length} certification{data.certifications.length > 1 ? 's' : ''} répertoriée{data.certifications.length > 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
                {expandedProjSection === 'certifications' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedProjSection === 'certifications' ? 'auto' : 0, opacity: expandedProjSection === 'certifications' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Vos Certifications</span>
                    <button
                      type="button"
                  ...
                  {data.certifications.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-4 italic">Aucune certification ajoutée.</p>
                  ) : (
                    <div className="space-y-2">
                      {data.certifications.map((cert, index) => (
                        <div key={cert.id} className="flex items-center gap-2 bg-slate-50/50 p-1.5 rounded-xl border border-slate-100">
                          <input
                            type="text"
                            value={cert.title}
                          ...
                          <input
                            ...
                            className="w-32 text-xs p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none bg-white transition-all font-medium"
                          />
                          <input
                            type="text"
                            value={cert.date}
                            ...
                            placeholder="Année"
                            className="w-20 text-xs p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-slate-900/20 focus:border-slate-900 focus:outline-none bg-white transition-all font-medium"
                          />
                          <button
                            type="button"
        ...
        {activeTab === 'customization' && (
          <div className="space-y-4">
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedCustSection(expandedCustSection === 'presets' ? null : 'presets')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">✨ Préréglages en 1 Clic</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">Design complet coordonné</p>
                  </div>
                </div>
                {expandedCustSection === 'presets' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedCustSection === 'presets' ? 'auto' : 0, opacity: expandedCustSection === 'presets' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedCustSection(expandedCustSection === 'layouts' ? null : 'layouts')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">📐 Modèle de Mise en Page (10 Thèmes)</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">{data.settings.templateId.toUpperCase()} actif</p>
                  </div>
                </div>
                {expandedCustSection === 'layouts' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedCustSection === 'layouts' ? 'auto' : 0, opacity: expandedCustSection === 'layouts' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white">
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedCustSection(expandedCustSection === 'colors' ? null : 'colors')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">🌈 Palette de Couleurs & En-tête</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">Couleur d&apos;accent & styles</p>
                  </div>
                </div>
                {expandedCustSection === 'colors' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedCustSection === 'colors' ? 'auto' : 0, opacity: expandedCustSection === 'colors' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-800 uppercase tracking-wider mb-2 flex items-center gap-2">
                      Nuance d&apos;accentuation (12 Nuances)
                    </label>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <div className="flex items-center gap-2 ml-2 border-l border-slate-200 pl-3">
                        <span className="text-xs text-slate-600 font-medium">Sélecteur:</span>
                        <input
                          type="color"
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedCustSection(expandedCustSection === 'photo' ? null : 'photo')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">📸 Options de la Photo de Profil</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">Affichage, forme et dimensions</p>
                  </div>
                </div>
                {expandedCustSection === 'photo' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedCustSection === 'photo' ? 'auto' : 0, opacity: expandedCustSection === 'photo' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white space-y-4">
                  <div className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-200/60">
                    <span className="text-xs font-semibold text-slate-700">Visibilité de la photo</span>
                    <button
                      type="button"
                      onClick={() => {
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full text-xs p-2 border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-slate-900/10"
                        >
            ...
            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-2xs">
              <button
                type="button"
                onClick={() => setExpandedCustSection(expandedCustSection === 'typography' ? null : 'typography')}
                className="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100/80 border-b border-slate-200/60 transition-all text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-200/50 rounded-xl text-slate-700">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800">🔤 Typographie & Espacement</h4>
                    <p className="text-[10px] text-slate-500 mt-0.5 font-medium">Choix de polices & densités</p>
                  </div>
                </div>
                {expandedCustSection === 'typography' ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
              </button>

              <motion.div
                initial={false}
                animate={{ height: expandedCustSection === 'typography' ? 'auto' : 0, opacity: expandedCustSection === 'typography' ? 1 : 0 }}
                transition={{ duration: 0.2, ease: 'easeInOut' }}
                className="overflow-hidden"
              >
                <div className="p-4 bg-white space-y-4">
